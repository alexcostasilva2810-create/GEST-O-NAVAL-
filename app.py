...
if 'db_comb' not in st.session_state:
    st.session_state.db_comb = pd.DataFrame(columns=[
        ...
        'Plano_H', 'LH_RPM', 'H_Manobra', 'LH_Manobra', 'H_MCA', 'Transf',
...
if aba == "⛽ Combustível":
    ...
    if st.button("✅ SALVAR NOVO ABASTECIMENTO", use_container_width=True, type="primary"):
        nova_l = pd.DataFrame([{
            ...
            "LH_Manobra": 0.0, "H_MCA": h_mca, "Transf": transf_balsa,
...
#----------------------------------#
# BLOCO 2 - MEDIÇÃO DIÁRIA (VÍDEO 2)
#----------------------------------#
    ...
    with c1:
        ...
        trecho = st.text_input("TRECHO / SERVIÇO")
    with c2:
        h_ini = st.number_input("HORÍMETRO INICIAL", min_value=0.0, step=0.1)
    ...
    with c3:
        media_lh = st.number_input("MÉDIA L/H", value=0.0, step=0.1)
        consumo_motor = horas_trab * media_lh
        st.metric("CONSUMO MOTOR (L)", f"{consumo_motor:.2f} L")
    with col4:
        mca_h = st.number_input("HORAS MCA", value=0.0, step=0.1)
    ...
    if st.button("🚀 FINALIZAR E ATUALIZAR TABELA GERAL", use_container_width=True, type="primary"):
        nova_linha_comb = pd.DataFrame([{
            ...
            "LH_Manobra": 0.0, "H_MCA": mca_h, "Transf": 0.0,
...
#----------------------------------#
    ...
    st.write("Área para lançamento de provisões.")
